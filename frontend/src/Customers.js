...
const api = axios.create({
    baseURL: `http://172.16.250.233:8080`
})
...
function Customers() {
    ...
    const handleRowUpdate = (newData, oldData, resolve) => {
        ...
        if (newData.age === undefined) {
            errorList.push("Please enter age")
        }
        ...
        if (errorList.length < 1) {
            api.put("/api/customer/" + newData.id, newData)
                .then(res => {
    ...
    const handleRowDelete = (oldData, resolve) => {
        api.delete("/api/customer/" + oldData.id)
            .then(res => {
    ...
    const handleDeleteRows = (event, rowData) => {
        ...
        for (const value in rowData) {
            api.delete("/api/customer/" + rowData[value].id)
                .then(res => {

...
function Users() {
    ...
    const handleRowUpdate = (newData, oldData, resolve) => {
        ...
        if (newData.dob === undefined) {
            errorList.push("Please enter data")
        }
        if (newData.bloodGroup === undefined) {
            errorList.push("Please enter group")
        }

        if (errorList.length < 1) {
            api.put("/api/user/" + newData.id, newData)
                .then(res => {
    ...
    const handleRowDelete = (oldData, resolve) => {
        api.delete("/api/user/" + oldData.id)
            .then(res => {
    ...
    const handleDeleteRows = (event, rowData) => {
        ...
        for (const value in rowData) {
            api.delete("/api/user/" + rowData[value].id)
                .then(res => {
